...
//!
//! ```sh
//! cargo test -p pecu-chain --test live_probe -- --ignored --nocapture
//! ```
//!
...
use pecu_chain::{Network, Node, NodeStatus};
...
fn a_public_testnet_node_answers_and_says_which_chain_it_is() {
    let (result, latency) = pecu_chain::probe(TESTNET);
...
fn an_unreachable_endpoint_backs_off_instead_of_hanging() {
    let (result, elapsed) = pecu_chain::probe("https://node.invalid.pecu.test");
    let error = result.expect_err("a nonexistent host must not answer");

    let mut node = Node::builtin(0, "dead", "https://node.invalid.pecu.test");
