...
fn draft(name: &str) -> chainvue_protocol::CurrencyDraft {
...
async fn wait_for_pending(
    ...
    ready: impl Fn(Option<&chainvue_protocol::LaunchPendingVm>) -> bool,
) -> chainvue_protocol::LaunchPendingVm {
    tokio::time::timeout(std::time::Duration::from_secs(30), async {
        loop {
            match events.recv().await {
...
async fn a_currency_waiting_for_its_name_outlives_the_wallet_that_configured_it() {
    ...
    let (dispatcher, mut events) =
        chainvue_core::start(&tokio::runtime::Handle::current(), config(home.clone()));
    ...
    let (_resumed, mut events) =
        chainvue_core::start(&tokio::runtime::Handle::current(), config(home));
...
async fn a_resumed_launch_is_signed_but_waits_for_a_press() {
    ...
    let path =
        chainvue_core::paths::Paths::new(home.clone(), &chainvue_chain::Network::Testnet, true)
            .launch();
    let mut intent = chainvue_core::launch::Intent::open(path);
    ...
    let mut pending: Option<chainvue_protocol::LaunchPendingVm> = None;
    ...
    dispatcher.send(Command::Refresh(chainvue_protocol::RefreshScope::All));
