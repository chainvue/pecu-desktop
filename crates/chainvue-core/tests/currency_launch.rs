...
use chainvue_core::currency;
use chainvue_keystore::{NewKey, Vault};
...
struct Wallet {
    ...
    nodes: chainvue_chain::NodeManager,
...
fn wallet() -> Wallet {
    ...
    let mock = chainvue_mock::MockChain::demo(std::slice::from_ref(&address))
        .expect("the demo script builds");
    let chain = chainvue_chain::Chain::Mock(mock.clone());
...
fn parent(chain: &chainvue_chain::Chain) -> verus_sdk::currency::CurrencyId {
...
fn draft(identity: &str) -> chainvue_protocol::CurrencyDraft {
