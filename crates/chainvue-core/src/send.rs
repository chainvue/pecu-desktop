...
use chainvue_chain::{Chain, SpendPermit};
use chainvue_keystore::{Vault, VaultError};
use chainvue_protocol::{ReviewOutputVm, SendDraft, SendReviewVm};
...
pub fn validate(draft: &SendDraft, spendable: Amount) -> chainvue_protocol::DraftValidationVm {
...
/// `chainvue_chain::permit`, and there is no other route to a `Broadcaster` in
/// this application.
///
