...
pub fn row(summary: &CurrencySummary, tip: u32) -> chainvue_protocol::CurrencyVm {
    let kind = Kind::of(summary.options);
    chainvue_protocol::CurrencyVm {
...
pub fn problems(draft: &chainvue_protocol::CurrencyDraft, tip: u32) -> Vec<Problem> {
    let mut out = Vec::new();
...
fn identity_problems(draft: &chainvue_protocol::CurrencyDraft, out: &mut Vec<Problem>) {
...
/// acted on.
pub(crate) fn reserve_label(reserve: &chainvue_protocol::ReserveDraft) -> &str {
...
fn basket_problems(draft: &chainvue_protocol::CurrencyDraft, out: &mut Vec<Problem>) {
...
fn nft_problems(draft: &chainvue_protocol::CurrencyDraft, out: &mut Vec<Problem>) {
    if !draft.reserves.is_empty() {
        out.push(Problem::stop(
...
fn token_problems(draft: &chainvue_protocol::CurrencyDraft, out: &mut Vec<Problem>) {
    if !draft.reserves.is_empty() {
        out.push(Problem::stop(
...
fn start_problems(draft: &chainvue_protocol::CurrencyDraft, tip: u32, out: &mut Vec<Problem>) {
...
pub fn check(
    ...
    ticker: &str,
    identity_name: &str,
) -> chainvue_protocol::CurrencyDraftVm {
    let kind = Kind::named(&draft.kind);
    ...
    chainvue_protocol::CurrencyDraftVm {
        problems: found
            .into_iter()
            .map(|p| chainvue_protocol::CurrencyProblemVm {
                blocking: p.blocking,
                text: p.text,
            })
            .collect(),
...
pub fn choices(
    ...
    let rows = hits
        .into_iter()
        .take(SHOWN)
        .map(|(_, _, summary)| chainvue_protocol::CurrencyPickVm {
            name: summary.fully_qualified_name.clone(),
            address: summary.currency_id.clone(),
...
fn bar<'a>(
    ...
    whole: i64,
) -> Vec<chainvue_protocol::CurrencySliceVm> {
    let mut out = Vec::new();
    ...
    for (index, (label, part)) in parts.enumerate() {
        ...
        out.push(chainvue_protocol::CurrencySliceVm {
...
fn flow_step(label: &str, state: &str, costs: bool) -> chainvue_protocol::FlowStepVm {
    chainvue_protocol::FlowStepVm {
...
pub fn plan(draft: &chainvue_protocol::CurrencyDraft) -> Vec<chainvue_protocol::FlowStepVm> {
...
pub fn progress(step: crate::launch::Step) -> Vec<chainvue_protocol::FlowStepVm> {
...
fn preview(
    ...
    ticker: &str,
    identity_name: &str,
) -> Vec<chainvue_protocol::CurrencyFieldVm> {
    let field = |label: &str, value: String, permanent: bool| chainvue_protocol::CurrencyFieldVm {
        label: label.to_string(),
        value,
        permanent,
    };
...
pub fn prepare(
    chain: &chainvue_chain::Chain,
    vault: &chainvue_keystore::Vault,
    label: &str,
    identity: &str,
...
///
/// `chainvue_protocol::coins_u64` is the one rule; this only gets the count
...
fn sats_display(sats: i64) -> String {
    chainvue_protocol::coins_u64(u64::try_from(sats).unwrap_or(0))
}
...
mod tests {
    ...
    fn draft(kind: &str) -> chainvue_protocol::CurrencyDraft {
    ...
    fn reserve(name: &str, weight: &str) -> chainvue_protocol::ReserveDraft {
        reserve_at(name, &format!("i-{name}"), weight)
    }

    fn reserve_at(name: &str, address: &str, weight: &str) -> chainvue_protocol::ReserveDraft {
        chainvue_protocol::ReserveDraft {
    ...
    fn allocation(to: &str, amount: &str) -> chainvue_protocol::PreallocationDraft {
        chainvue_protocol::PreallocationDraft {
