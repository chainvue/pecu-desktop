...
//!
//! Two copies of ChainVue on the same home do not merely draw the same figures
...
pub fn acquire(home: &Path) -> Result<Guard, Busy> {
    ...
    let path = home.join("chainvue.lock");
    ...
    match file.try_lock() {
        ...
        Err(std::fs::TryLockError::WouldBlock) => {
            tracing::warn!(path = %path.display(), "another ChainVue holds this wallet");
