...
fn small(net: SignedAmount) -> bool {
    net.magnitude().to_sat() < pecu_protocol::SATS_PER_COIN as u64 / 10
}
...
///
/// The rule itself is in [`pecu_protocol::format`], because the interface
...
pub fn coins(amount: Amount) -> String {
    pecu_protocol::coins_u64(amount.to_sat())
}
