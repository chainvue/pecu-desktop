...
//! chainvue-mock`", and this crate's manifest is written to make that true: it
//! takes `verus-rpc` **without** the `http` feature.
//!
//! Inside this workspace it is false anyway. Cargo unifies features across
//! normal dependencies, and `chainvue-chain` needs `verus-rpc/http` for the
...
fn no_source_file_names_a_transport() {
    ...
    for entry in std::fs::read_dir(&src).expect("chainvue-mock/src must exist") {
...
fn the_manifest_still_declines_the_http_feature() {
    ...
    assert!(
        ...
        "chainvue-mock must not ask for verus-rpc's http feature",
    );
}
